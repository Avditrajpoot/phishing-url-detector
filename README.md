...
text phishing-url-detector/ │ ├── data/ │ └── urls.csv # Synthetic dataset of URLs │ ├── src/ │ ├── generate_dataset.py # Creates dataset │ ├── utils.py # Feature extraction │ ├── train_model.py # Trains Decision Tree model │ └── predict_url.py # Predicts if a URL is phishing │ ├── model/ │ └── url_phish_dt.pkl # Saved trained model │ ├── report/ │ └── report.md # Project report │ └── requirements.txt How to Run the Project

Install dependencies

bash pip install -r requirements.txt Generate the dataset

bash python src/generate_dataset.py This will create a fresh urls.csv file inside the data folder.

Train the model

bash python src/train_model.py This trains the Decision Tree model and saves it in the model folder.

Test a URL

To check a URL:

bash python src/predict_url.py "<your_url_here>" Example:

bash python src/predict_url.py "http://paypal.verify-login.xyz/confirm" Output will show:
